...
def transformer_pipeline(
    ...
    **selector_params,
    ...
    # Permitir legacy: selector_params={'selector_params': {...}}
    ...
    # 4) Eliminación de redundancias (opcional)
    ...
    # 6) Selección de características (opcional)
    X_sel = X_df
    selector_name = None
    ...
    if selector is not None:
        ...
        if sel in ("bso", "bso-cv", "bsocv"):
            ...
            # Pre-escalado SOLO para el fitness CV del selector (está OK)
            X_scaled_all = scale_features(X_df.values, scaler_type=scaler_type)
            ...
            penalty_weight = selector_params.get("penalty_weight", 0.1)  # alineado al monolítico
    ...
    # 7) Split y escalado correcto para el MODELO FINAL (evitar fuga)
    X_tr_raw, X_te_raw, y_train, y_test = split_data(X_sel.values, y)
    X_train, X_test, _scaler = scale_train_test(X_tr_raw, X_te_raw, scaler_type=scaler_type)
